...
class HashTable:
    ...
    def get_value(self, key):
        ...
        node = self._data[hash(key)%self.number_of_slots]
...
def hash_result_finder(tested, quarantined, load_factor=0.5):
    ...
    for q_name in quarantined:
        ...
        if len(tested) > 0:
            nhi_and_result = hash_table.get_value(q_name)
            if nhi_and_result is not None:
                results[-1] = ((q_name, nhi_and_result[0], nhi_and_result[1]))
...
def my_tests():
    ...
    print('Cee\'s value =', table.get_value(Name('Cee')))

    table2 = HashTable(11)
    print('Joe\'s value =', table2.get_value(Name('Joe')))
    
    tested = []
    quarantined = []
    print(hash_result_finder(tested, quarantined))
