...
class BinarySearchTree(object):
    ...
    def _pop_min_recursive(self, subtree_root):
        ...
        if subtree_root.left.left is None:
            min_value = subtree_root.left.value
            subtree_root.left = subtree_root.left.right
        else:
            min_value = self._pop_min_recursive(subtree_root.left)
        # ===end student section===
        return min_value
...
if __name__ == '__main__':
    ...
    for num in [13, 9, 8, 10, 11, 15, 18, 13, 16]:
        tree.insert(num)
        
    print(tree.in_order_items())
    print(tree.pre_order_items())
    print(tree.post_order_items())
    
    alist = load_file("list4.txt")
    my_fave_bst = BinarySearchTree()
    for num in alist:
        my_fave_bst.insert(num)
    
    print(my_fave_bst)
    print(my_fave_bst.in_order_items())
    my_fave_bst.remove(my_fave_bst.root.value)
    print(my_fave_bst)