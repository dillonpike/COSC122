...
class MinHeap(Heap):
    ...
    def pop_min(self):
        ...
        # ---start student section---
        if len(self) > 1:
            minimum = self._items[1]
            self._items[1] = self._items.pop()
            self._sift_down(1)
        elif len(self) == 1:
            minimum = self._items.pop()
        else:
            minimum = None
        return minimum
        # ===end student section===
    ...
    def _sift_down(self, index):
        ...
        if (index * 2) <= len(self):
            left = 2 * index
            right = left + 1
            if len(self) >= right and self._items[right] < self._items[left]:
                smallest = right
            else:
                smallest = left
...
if __name__ == '__main__':
    ...
    alist = load_file("list1.txt")
    h = MinHeap()
    for num in alist:
        h.insert(num)
    print(h)
    
    alist2 = load_file("list2.txt")
    h2 = MinHeap()
    for num in alist2:
        h2.insert(num)
    for i in range(5):
