...
class Max_3_Heap(Heap):
    ...
    def _sift_down(self, index):
        ...
        # ---start student section---
        children = []
        if len(self) >= (index * 3 - 1):
            for i in range(min(len(self) - (index*3 - 2), 3)):
                children.append((self._items[index*3 - 1 + i], index*3 - 1 + i))
            children.sort()
            largest = children[-1][1]
    ...
    def validate(self):
        ...
        for i in range(2, len(self._items)):
            if self._items[round(i/3)] < self._items[i]:
                return False
        return True        
        # ===end student section===
