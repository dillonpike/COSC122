...
class QuadraticHashTable():
    ...
    def store(self, item):
        ...
        # ---start student section---
        slot = self._hash(item)
        if self._data[slot] is not None:
            slot = self._next_free_slot(slot)
        self._data[slot] = item
        # ===end student section===
        self.n_items += 1
    ...
    def __contains__(self, item):
        ...
        # ---start student section---
        for try_number in range(self.n_slots):
            if self._data[current_index] == item:
                return True
            elif self._data[current_index] is None:
                break
